...
settings.version = 'pixel_test'
settings.name = 'EBEX-IDS'
...
settings.coma_corr = False
if settings.coma_corr:
  settings.version = settings.version + '_coma_corrected'

settings.aperture_radius = 0.7  # aperture radius in meters (2.5 meter primary = 1.25m radius)  !!! doesn't matter for this toy model??
settings.f_number = 0.1  # set arbitrarily small to force all power through stop.

settings.edge_db = 40   # edge taper on stop in dB.  May later be calculated from pixel sizes.
...
settings.mission_length = 4 * 0.95  # years at 90% effeciency
settings.sky_area = 41253 # deg (full sky)

# scale emissivity with frequency or not
settings.emiss_rt_GHz = False  #  True  #  
...
settings.use_edge_dB = False ## this means calculate pixel sizes from edge taper given earlier.
...
settings.elements_path = os.path.join(settings.base_path,
                 'inputs/EBEX_IDS_pixel.csv')  # all telescope surfaces, lenses, etc.
...
settings.bands_path = os.path.join(settings.base_path,
                 'inputs/EBEX_IDS_bands.csv')  # csv of bands.
