...
settings = Class()
# Run time paramters
settings.freq = '45_GHz'
settings.version = 'Epic_30K'

# Telescope/ Receiver Optical Parameters
settings.band = [38.3,51.8]  # GHz
...
settings.safety_factor = 5  # Unitless, ratio of p_sat to p_opt
...
settings.elements_path = os.path.join(settings.base_path,
                 'inputs/epic_check/epic_30K.csv')  # all telescope surfaces, lenses, etc.

settings.elements_out_path = os.path.join(settings.base_path,
                 'outputs/epic_check/%s_%s_elements_out.csv ' %(settings.freq, settings.version))  # data that gets saved.
