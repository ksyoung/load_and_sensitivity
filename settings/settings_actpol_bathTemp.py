...
settings.version = 'actpol_approxx'
settings.name = 'bathTemps'
...
settings.coma_corr = False #True
if settings.coma_corr:
  settings.version = settings.version + '_coma_corrected'

settings.aperture_radius = 3.  # aperture radius in meters (2.5 meter primary = 1.25m radius)
settings.f_number = 2.5  #

settings.edge_db = 5   # edge taper on primary mirror in dB.  can be calculated later from pixel sizes.
...
settings.mission_length = 400./24./365.25 #yrs  # 200 hours observing, at 100% effeciency, units here are YEARS!!
settings.sky_area = 600 #41253 # deg (full sky)

# scale emissivity with frequency or not
settings.emiss_rt_GHz =  True  #  False  #

# pixel size calculation parameters
settings.MCP =  False # assumes MCPs and finds pixel diameters, edge tapers, counts, etc.
...
settings.use_edge_dB = True # False # # this means calculate pixel sizes from edge taper given earlier.
settings.use_D_px = False #True  # # use D_px from bands.csv import 
...
settings.x_fp_diameter = 0.15  #
settings.y_fp_diameter = 0.15  #
...
settings.n = 2.5            # thermal power law exponent (EBEX was ~2)
...
settings.readout_type = 'FDM' # 'FDM'  # readout to use for main calculation. All values are recorded, but this readout goes into the final noise number.
...
settings.elements_path = os.path.join(settings.base_path,
                 'inputs/elements_actpol_bathTemp.csv')  # all telescope surfaces, lenses, etc.
...
settings.bands_path = os.path.join(settings.base_path,
                 'inputs/bands_actpol_bathTemp.csv')  # csv of bands.
				 

settings.FP_areas_path = os.path.join(settings.base_path,
                 'inputs/FP_areas_%s_dummy.csv' %settings.version)  # csv of FP areas.
				 #'inputs/FP_areas_3bands_%s_dummy.csv' %settings.version)  # csv of FP areas.
